...
object pos {
  ...
  def main(args: Array[String]): Unit = {
    ...
    var sig_p = Uniform(3-delta_s,3+delta_s).sample(2).toArray
    ...
    var rho_p = Uniform(0.0-delta_r,0.0+delta_r).sample(1).toArray
  ...
  //def one_step(data: Double, states: Vector[List[Int]], gamma_1: Array[Double],
  //             b: Array[Double], betas: Array[Double], sig: Array[Double], rho: Double): Unit = {
    // make binary version of all state combinations (0,0), .... (2^kbar(0),2^kbar(1))
    ...
    //val states_probs = states_vec.map(v=> v.map(vv=>(vv,0.0)).toMap).reduce(_++_)
  //}
  ...
  def one_step(Mt:scala.collection.immutable.Map[(String, String),Double],state: IndexedSeq[List[String]],kbar: Array[Int],
    ...
    val switch_prob = lik_andswitches.map(v=>v.map(vv => (vv._4(1) zip (0 until vv._4(1).length)).map(vvv=> math.log(vvv._1*gammas(1)(vvv._2)+(1-vvv._1)*(1-gammas(1)(vvv._2)))).sum+
      (vv._4(0) zip (0 until vv._4(0).length)).map(vvv=> math.log(vvv._1*(sigmoid(logit(gammas(0)(vvv._2))-(beta(vvv._2) zip gammas(1)).map(b=>b._1*b._2).sum+
        (beta(vvv._2) zip vv._4(1)).map(bb=>bb._1*bb._2).sum))+(1-vvv._1)*(1-(sigmoid(logit(gammas(0)(vvv._2))-(beta(vvv._2) zip gammas(1)).map(b=>b._1*b._2).sum+
